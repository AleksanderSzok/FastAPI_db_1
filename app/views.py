...
from . import schemas
from .crud import AsyncDbManager
...
async def get_suppliers(db: AsyncSession = Depends(get_db)):
    suppliers = await AsyncDbManager(session=db).get_suppliers()
...
async def get_shipper(supplier_id: PositiveInt, db: AsyncSession = Depends(get_db)):
    db_supplier = await AsyncDbManager(session=db).get_supplier(supplier_id)
...
async def get_products(supplier_id: PositiveInt, db: AsyncSession = Depends(get_db)):
    db_products = await AsyncDbManager(session=db).get_product(supplier_id)
    if not db_products:
...
async def create_supplier(
    ...
    return await AsyncDbManager(session=db).post_supplier(item=item)
...
async def update_supplier(
    ...
    update_item = await AsyncDbManager(session=db).put_supplier(
...
async def delete_supplier(supplier_id: PositiveInt, db: AsyncSession = Depends(get_db)):
    supplier = await AsyncDbManager(session=db).get_supplier(supplier_id=supplier_id)
    if not supplier:
        raise HTTPException(status_code=404)
